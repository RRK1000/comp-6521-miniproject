...
# 4
    ...
    def processBitmapJoin(self, conn, clauses, relationList) -> list:
        ...
        # should be a dictionary of dictionary of all the bitmaps generated by individual condition
        allBitmapsCombined={}
        ...
        for cond in conditionList:
            ...
            conditionCounter+=1
            allBitmapsCombined[conditionCounter]=[]
                    ...
                    if key in rhsBitmap.keys():
                        ...
                        for pos1R in indexOf1R:
                            for pos1S in indexOf1S:
                                ...
                                allBitmapsCombined[conditionCounter].append(tuple2+tuple1)
                                
                                # arr = []
                                # print(tuple1)
                                # for id in projectionIDX:
                                #     if id:
                                #         arr.extend([tuple1[id], tuple1[len(tuple1)-2]])
                                # print(arr)
            ...
            #     print(type(tmpRhs))
            #     tempBounds=tmpRhs.replace("(","").replace(")","").replace(" ","").split(",")
            #     lowerBound=int(tempBounds[0])
            #     upperBound=int(tempBounds[1])

            #     lhsBitmap = bitmap.extractBitmap("bitmap_index_"+lhs+".csv")
            #     for key, bit1 in lhsBitmap.items():
            #         if key>=lowerBound and key<=upperBound:

        print("1ultimate print")
        ultimateRows=set(allBitmapsCombined[0]) & set(allBitmapsCombined[1])
        print(ultimateRows)
